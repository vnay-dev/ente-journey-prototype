...
type Props = {
  ...
  as?: "h2" | "p";
  /** Green glitter sweep on the current value — no fade/reveal swap */
...
export function EnsuFieldReveal({
  ...
  const highlightOnlyActive = active && highlightOnly;
  const canAnimate =
    active && !highlightOnly && Boolean(previousValue) && previousValue !== value;

  const displayPhase: Phase = canAnimate && phase === "idle" ? "sweep" : phase;

  useEffect(() => {
    if (!highlightOnlyActive) {
      return;
    }

    const doneTimer = window.setTimeout(() => {
      onComplete?.();
    }, TIMING.highlightOnly);

    return () => window.clearTimeout(doneTimer);
  }, [animationKey, highlightOnlyActive, onComplete]);
