...
export function ItineraryPhotoPreview({
  ...
  useEffect(() => {
    ...
    function handleDemoAdvance(event: Event) {
      const direction = (event as CustomEvent<{ direction?: "left" | "right" }>).detail
        ?.direction;
      ...
      setIndex((current) => {
        if (direction === "left") {
          return Math.min(current + 1, photoIndices.length - 1);
        }
        return Math.max(current - 1, 0);
      });
    }

    stage.addEventListener("ente-demo-preview-advance", handleDemoAdvance);
    return () => stage.removeEventListener("ente-demo-preview-advance", handleDemoAdvance);
  }, [open, photoIndices.length]);
  ...
  return (
    <div className={styles.overlay} role="dialog" aria-modal="true" aria-label={location}>
      ...
      <div
        ...
        data-demo-target="photo-preview-stage"
